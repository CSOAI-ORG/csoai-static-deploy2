#!/usr/bin/env python3
"""Sovereign DPA Generator — auto-generate a UK GDPR + EU GDPR compliant
Data Processing Agreement. Outputs: sovereign-dpa-2026-07-13.md
...
OUT = SC / 'sovereign-dpa-2026-07-13.md'

now = datetime.now(timezone.utc).isoformat()
md = f'''# CSOAI Ltd — Data Processing Agreement (DPA)
...
import hashlib
sigil = hashlib.sha256(f'dpa|{now}'.encode()).hexdigest()[:32]
with open(SC / 'SIGIL_LOG.txt', 'a') as f:
    f.write(f'{now} | {sigil} | M|JEEVES|csoai|DPA-GENERATOR. dpa_bytes={len(md)}\n')