...
con.execute("DELETE FROM files WHERE root='iCloud'")  # clear the partial stat-only rows
...
for dp,dns,fns in os.walk(IC,topdown=True):
    ...
    for fn in fns:
        ...
        sig+=1
    if n%20000<300: con.commit()
con.commit()
print(f"iCloud DONE: total={n} signal={sig} elapsed={int(time.time()-t0)}s")
con.close()
