...
**0.3 — Real sovereign weights on free GPU (path proven this session).** [V + V-run]
Unsloth QLoRA: **Qwen3-14B fine-tunes on a free Colab 16GB T4**; the **Qwen3-30B-A3B MoE fits in 17.5 GB**. Our Colab-T4 pipeline now works end-to-end (this session: `HF_HUB_DISABLE_XET=1` + GitHub-jsonl dataset + `return_dict=False` fixes). Train one genuinely-useful sovereign expert, not a toy.
· Owner-gated: HF token. · Caveat: QLoRA ≠ full FT; T4 caps context; capability bounded by our modest raw numbers — **the value is governance, not the benchmark.**
