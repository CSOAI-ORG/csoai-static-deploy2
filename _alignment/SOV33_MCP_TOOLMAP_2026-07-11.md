...
## LIVE, ran clean (4): sovereign_health_check, sovereign_rundown, sovereign_ingest_run*, vault_stats
  *sovereign_ingest_run is STATE-CHANGING — VERIFIED output = corpus-build ONLY {sources:1093, corpus_mb:8.55, qa_pairs:47}. An OLM-retrain is in the tool's DESCRIPTION but was NOT confirmed by the call's return (no version bump / training metric). Ran live this session. Do NOT classify-probe it with no-args again.
