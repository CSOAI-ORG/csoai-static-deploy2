...
3. **MEOK Labs ring sweep** (sov33_tri_owem.py, their lane): diverse-R5 wins on BOTH clean (0.862) and containment (0.734);
...
**LAW (now triple-confirmed): decorrelation (diverse lineages) > redundancy (more identical nodes). Ring size is the expensive lever; lineage diversity is the cheap, higher-value one.**

## HONEST reconciliation flag (do NOT silently pick one)
- TWO files now encode this: my sov33_triangle_owem.py (3-around-1, sweepable ratios) and MEOK's sov33_tri_owem.py (R3/5/7/9 ring, diverse-R5 shipped).
