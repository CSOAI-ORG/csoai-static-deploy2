...
try:
    ...
    sigil = None   # signing best-effort; Merkle root is the primary integrity anchor
...
AGENTS = os.environ.get("PL_AGENTS", "llm")        # "stub" = deterministic (no LLM), "llm" = real
_stub = random.Random(SEED ^ 0x5A5A)               # seeded noise for stub agents
...
def classify(inc):
    if AGENTS == "stub":   # deterministic: ~15% error vs ground truth -> realistic FP/FN
        pred = inc["gt_is_major"] if _stub.random() > 0.15 else (not inc["gt_is_major"])
        return bool(pred), {"is_major": bool(pred), "rationale": "stub-agent", "_stub": True}
...
def report(inc, town):
    if AGENTS == "stub":   # automated template fills all fields; manual drops ~2 (simulated incompleteness)
        rep = {f: f"synthetic-{f}-{inc['incident_id']}" for f in REQUIRED_INITIAL_FIELDS}
        if town == "B_control":
            for f in _stub.sample(REQUIRED_INITIAL_FIELDS, 2):
                rep.pop(f, None)
        return rep
...
def run_town(town, inc):
    ...
    rep = report(inc, town) if reported_major else {}
...
def main():
    ...
    result = {
        ...
        "agents": AGENTS, "model": (AGENT_MODEL if AGENTS == "llm" else "deterministic-stub"),
        "seed": SEED, "n_incidents": N, "n_episodes": len(eps),
    ...
    try:
        ...
        result["sigil"] = sigil.emit(sigil.derive(seed_b, "keystone/policy-lab"),
                                     json.dumps(result, sort_keys=True, default=str)) if sigil else None
    except Exception as e:
        result["sigil"] = None
        result["sigil_note"] = f"signing unavailable ({e}); merkle_root is the primary integrity anchor"
