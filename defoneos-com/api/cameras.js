...
async function windyCams(key, area) {
  const url = 'https://api.windy.com/webcams/api/v3/webcams?limit=50&include=images,location,player' + (area ? ('&q=' + encodeURIComponent(area)) : '');
...
export default async function handler(req, res) {
  ...
  try {
    ...
    else if (wk) { cams = await windyCams(wk, area); source = 'Windy Webcams · global public network'; }
    else {
    ...
    if (area && !region) cams = cams.filter(c => (c.name || '').toLowerCase().includes(area));
