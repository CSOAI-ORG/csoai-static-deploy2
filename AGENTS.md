...
<!-- newest at top -->
- [08:10 Claude] CLAIM VM king_hive (judge JURY deploy+test) + ~/clawd/sprint + sim_params integration — deploying heterogeneous 3-family local-jury judge with position-swap (king_jury.py, smoke-tested BEFORE wiring into runner), integrating the MSPB sim_params spec into the Policy Lab plan (flagging its FreeLLMAPI token-budget basis → must be local Ollama). VM-side internal only; not touching cert pipeline.
