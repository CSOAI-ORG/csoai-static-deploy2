...
def call_openai_compat(task: str, resource: Resource, api_key: str) -> Tuple[str, bool]:
    ...
    if "{model}" in endpoint:
        endpoint = endpoint.replace("{model}", resource.model)
    ...
    def snapshot(self) -> Dict[str, Any]:
        with self._lock:
            ...
            for rid, s in self._stats.items():
                ...
                d["avg_latency_ms"] = s.avg_latency_ms
                out[rid] = d
            return out
    ...
    def route(self, task: str, route: str = "auto", resource_override: str = "") -> TaskResult:
        ...
        for res_id in chain:
            ...
            if is_err:
                ...
                print(f"    x {resource.id}: {resp[:120]}")
                error = resp
                continue
            ...
            print(f"    OK {resource.name} ({elapsed_ms:.0f}ms)")
            break

        if not success:
            final_resource = fallbacks_tried[-1] if fallbacks_tried else primary
            print(f"  x All resources failed for: {task[:60]}")
...
def cmd_batch(args):
    ...
    for r in results:
        ...
        print(f"  {status:4s} [{r.task_type:12s}] {r.final_resource:18s} {r.latency_ms:7.0f}ms  {r.task[:60]}")
