...
# D30-B PATCH (2026-06-16): /v1/best-of-n-generate
# Wraps /v1/best-of-n with a king_ask generator chain.
# Caller passes a prompt + n; this endpoint calls king_ask n times, then
# /v1/best-of-n with the candidates.
@app.post("/v1/best-of-n-generate")
async def best_of_n_generate_endpoint(request: Request):
    import httpx
    body = await request.json()
    prompt = body.get("prompt", "")
    n = int(body.get("n", 3))
    fan_out = bool(body.get("fan_out", False))
    verifiers = body.get("verifiers", ["json_valid", "schema_keys"])
    king_url = body.get("king_url", "http://localhost:8077/mcp")
    # Generate n candidates via king_ask
    candidates = []
    async with httpx.AsyncClient(timeout=60) as client:
        for i in range(n):
            try:
                r = await client.post(king_url, json={
                    "jsonrpc": "2.0", "id": f"bn{i}", "method": "tools/call",
                    "params": {"name": "king_ask",
                               "arguments": {"message": prompt, "fan_out": fan_out, "k": 1}}
                })
                d = r.json()
                text = d.get("result", {}).get("content", [{}])[0].get("text", "{}")
                j = json.loads(text)
                candidates.append(j.get("reply", "") or "[no reply from king_ask]")
            except Exception as e:
                candidates.append(f"[generation error: {e}]")
    # Score via /v1/best-of-n
    v = make_verifier(required_keys=verifiers)
    result = best_of_n(candidates, v)
