// Cloudflare Pages Function — converted from api/eat-tick-v2.js
...
export async function onRequest(context) {
  ...
  async function runTick(task, opts = {}) {
    const tick_id = 'eatv2_' + randomBytes(8).toString('hex');
    const state = {
      tick_id,
      task,
    ...
    if (expected && provided !== expected) {
      return new Response(JSON.stringify({ status: 'error', error: 'Invalid X-Send-Key' }), { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    if (request.method === 'GET') {
      return new Response(JSON.stringify({
        ...
        tick_id: 'preview_' + randomBytes(4).toString('hex'),
