// Cloudflare Pages Function — converted from api/crown-rfq.js
...
export async function onRequest(context) {
  ...
  function hmac(payload, key = HMAC_KEY) {
    return createHmac('sha256', key).update(typeof payload === 'string' ? payload : JSON.stringify(payload)).digest('hex');
  }

  function sha512(payload) {
    return createHash('sha512').update(typeof payload === 'string' ? payload : JSON.stringify(payload)).digest('hex');
  }

  function newRfqId() {
    return 'crown_' + Date.now().toString(36) + '_' + randomBytes(4).toString('hex');
  }
  ...
  async function fanoutEmail(record) {
    ...
    if (request.method === 'GET') {
      return new Response(JSON.stringify({
        ...
        rfq_id: 'preview_' + randomBytes(4).toString('hex'),
        schema: {
    ...
    if (request.method !== 'POST') return new Response(JSON.stringify({ status: 'error', error: 'Method not allowed' }), { status: 405, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });

    let body = await request.json();
    if (typeof body === 'string') { try { body = JSON.parse(body); } catch { return new Response(JSON.stringify({ status: 'error', error: 'Invalid JSON' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }); } }
    if (!body || typeof body !== 'object') body = {};

    // Honeypot — bots
    if (body.honeypot) {
      return new Response(JSON.stringify({ status: 'ok', sigil: sha512('bot-suppressed-' + Date.now()), timestamp: new Date().toISOString(), rfq_id: 'bot_' + randomBytes(4).toString('hex') }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    // GDPR gate
    if (body.gdpr_consent !== true) {
      return new Response(JSON.stringify({ status: 'error', error: 'GDPR consent required', code: 'GDPR_CONSENT_MISSING' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    ...
    if (errors.length) {
      return new Response(JSON.stringify({ status: 'error', error: 'validation_failed', code: 'INVALID_RFQ', details: errors }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
