// Cloudflare Pages Function — converted from api/owem-cycle.js
...
export async function onRequest(context) {
  ...
  // DEFONEOS SOV3³ OWEM cycle endpoint (POST → one OOWM cycle)
  //
  // POST /api/owem-cycle
  //
  ...
  function mambaStateDelta(input, prevState = null) {
    const h = createHash('sha256').update(String(input)).digest();
    const fresh = [];
    for (let i = 0; i < 16; i++) {
      const byte = h[i * 2];
  ...
  function cycleId(input) {
    ...
    const cyclePayload = {
      ...
      context_hash: createHash('sha256').update(context || cid).digest('hex'),
      timestamp,
    };
