// Cloudflare Pages Function — converted from api/hermes-stream.js
...
export async function onRequest(context) {
  ...
  // DEFONEOS Hermes agentic layer SIGIL STREAM endpoint
  ...
  function readTail(filePath, n = 200) {
    try {
      const data = "" /* fs.readFileSync no-op */
  ...
  function normaliseSigil(line, idx, sourceLabel) {
    ...
    const sigil_id = o.sigil_id || o.sigil || o.digest
      || ('sigil-' + createHash('sha1').update(line).digest('hex').slice(0, 16));
    return {
      sigil_id,
  ...
  function computeBftHealth(latestTs, hasTally) {
    ...
    const rawLimit = parseInt(url.searchParams && url.searchParams.get("limit"), 10);
