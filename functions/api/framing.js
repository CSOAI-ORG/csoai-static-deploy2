// Cloudflare Pages Function — converted from api/framing.js
export async function onRequest(context) {
  ...
  const FRAMES = [
    ...
    const topic = (url.searchParams.get("topic") || '').toString();
    let frames = FRAMES;
    if (topic) frames = frames.filter(f => f.topic === topic);

    if (format === 'quote-cards') {
      return new Response(JSON.stringify({ format: 'quote-cards', count: frames.length, items: frames }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }
    if (format === 'markdown') {
    ...
    corsHeaders['Content-Type'] = 'text/markdown; charset=utf-8';
