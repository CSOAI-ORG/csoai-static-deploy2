// Cloudflare Pages Function — converted from api/welcome.js
export async function onRequest(context) {
  ...
  async function sendViaSmtp(email) {
    ...
    const providedKey = (request.headers['x-send-key'] || request.headers.authorization || '').replace(/^Bearer\s+/i, '').trim();
    ...
    if (!record.email || !record.sigil) return new Response(JSON.stringify({ error: 'record.email + record.sigil required' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    ...
    return new Response(JSON.stringify({ ok: true, delivered: true, result, email_subject: email.subject }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
}
