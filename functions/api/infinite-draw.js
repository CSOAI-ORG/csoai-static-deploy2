// Cloudflare Pages Function — converted from api/infinite-draw.js
...
export async function onRequest(context) {
  ...
  // DEFONEOS Infinite Drawing API endpoint
  //
  ...
  function initHives(graph) {
    ...
    if (op === 'draw') {
      ...
      if (!parent) return new Response(JSON.stringify({error: `Node ${nodeId} not found`}), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    ...
    return new Response(JSON.stringify({status: 'ok', op, ...result, sigil, timestamp: ts}), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
}
