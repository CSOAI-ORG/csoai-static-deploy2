// Cloudflare Pages Function — converted from api/train-distributed.js
...
export async function onRequest(context) {
  ...
  // DEFONEOS £0 distributed training dispatcher
  ...
  async function appendJob(record) {
    ...
    corsHeaders['X-Sovereign-Budget'] = 'GBP-0';
    ...
    const idMaterial = `${model}|${dataset}|${epochs}|${tabs.join(',')}|${timestamp}|${randomBytes(8).toString('hex')}`;
    const jobId = `train-${createHash('sha256').update(idMaterial).digest('hex').slice(0, 16)}`;
    const profile = MODEL_PROFILES[model];
