// Cloudflare Pages Function — converted from api/arena-vote.js
...
export async function onRequest(context) {
  ...
  // DEFONEOS SOV33 arena preference-vote endpoint
  // POST /api/arena-vote
  //
  ...
  async function appendLog(record) {
    ...
    const vote_id = randomBytes(6).toString('hex');
    const prompt_hash = createHash('sha256').update(prompt).digest('hex').slice(0, 16);
    ...
    const voter_sig = createHmac('sha256', HMAC_SECRET).update(JSON.stringify(voterSigPayload)).digest('hex');
