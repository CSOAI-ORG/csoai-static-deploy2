...
def _run(spec):
    # rates = extra governed block_rates to probe for the dose-response curve (the canonical
    # A_governed run below is always block_rate=1.0). Rates ride in the spec tuple — NOT a module
    # global — so they survive macOS 'spawn' workers. Extra runs feed `sweep` only; they never
    # touch episodes/A_crimes/B_crimes, so the published headline + chain stay byte-stable.
    district, seed, c, *rest = spec
    rates = rest[0] if rest else ()
    ...
    sweep = {r: sim.run_arm("A_governed", None, {"sig": ""}, None, sign=False,
                            district=district, seed=s, block_rate=r)["violations"] for r in rates}
    return (district, a["episodes"] + b["episodes"], a["violations"], b["violations"], sweep)
...
def cycle(st, priv, pool, ledger_path, status_path, state_path, host, rates=()):
    base = st["seed_cursor"]
    specs = [(d, base + i, c, rates) for d in sim.DISTRICTS for i in range(SEEDS_PER_CYCLE) for c in CONTAGION]
    ...
    if rates:
        # Signed dose-response: governed violations by Sovereign-Gate enforcement level.
        # "1.0" == A_crimes (canonical); B_crimes is the ungoverned control. Proves governance
        # works PROPORTIONALLY to enforcement — not "governed AI is crimeless by magic".
        sweep = {"1.0": ac}
        for r in rates:
            sweep[f"{r}"] = sum(x[4].get(r, 0) for x in res)
        entry["sweep"] = sweep
        entry["sweep_control_ungoverned"] = bc
...
def main():
    ...
    ap.add_argument("--sweep", action="store_true",
                    help="record the signed dose-response curve (governed crimes at extra enforcement levels)")
    ap.add_argument("--sweep-rates", default="0.0,0.5",
                    help="comma-separated extra block_rates to probe when --sweep is set (1.0 is always included)")
    args = ap.parse_args()
    rates = tuple(sorted({float(x) for x in args.sweep_rates.split(",") if x.strip()} - {1.0})) if args.sweep else ()
    ...
    try:
        while True:
            t0 = time.time(); e = cycle(st, priv, pool, ledger_path, status_path, state_path, host, rates=rates); dt = time.time() - t0
            sw = f" | dose {e['sweep']}" if e.get("sweep") else ""
            print(f"  [{host}] cycle {e['cycle']:>5} | {e['town_runs']} towns | {e['episodes']:,} eps {dt:.1f}s "
                  f"| gov {e['A_crimes']} / ungov {e['B_crimes']:,} | cum {e['cum_episodes']:,}{sw}")
            n += 1
