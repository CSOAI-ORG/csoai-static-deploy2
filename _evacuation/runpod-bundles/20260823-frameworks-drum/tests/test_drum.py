...
def main():
    ...
    check("kinds are valid", all(i["kind"] in ("framework", "charter", "regulation", "article", "sector", "benchmark") for i in items))

    # card coverage: every item has a card, no stale cards
    dirs = {"framework": "frameworks", "charter": "charters", "regulation": "regulations", "article": "articles", "sector": "sectors", "benchmark": "benchmarks"}
    cards = set()
