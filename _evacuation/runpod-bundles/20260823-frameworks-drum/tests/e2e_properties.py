...
def main():
    ...
    def p_archive_invariant():
        # content-addressed append: identical content appends exactly once (no dup regrowth),
        # and the test isolates itself in a temp store so it never pollutes production.
        import tempfile
        import sys as _s
        _s.path.insert(0, os.path.join(PACK, "archive"))
        import knowledge_archive as ka
        os.environ["KNOWLEDGE_ARCHIVE_INDEX"] = os.path.join(tempfile.mkdtemp(), "index.jsonl")
        before = ka.count()
        ka.append("property-test", "invariant-check", {"t": 1})
        after = ka.count()
        assert after == before + 1, (before, after)  # append-only: grows by exactly one
        ka.append("property-test", "invariant-check", {"t": 1})  # same content
        assert ka.count() == after, (after, ka.count())  # idempotent: no duplicate
        os.environ.pop("KNOWLEDGE_ARCHIVE_INDEX", None)
