---
license: apache-2.0
pretty_name: ProvBench — C2PA manifest survival (provenance axis, GSPC)
language:
  - en
tags:
  - benchmark
...
# ProvBench — the provenance axis of GSPC

> **C2PA manifest survival**, scored against frozen, corpus-anchored ground truth. **n = 15 items.**
...
Total **15**. Labels: `DESTROYED`, `SURVIVES`
...
|---|---:|---|
| mean difficulty | 0.562 | fraction of models answering an item correctly |
| spread (max−min) | 0.400 | how far this axis separates the best model from the worst |
| dead items | 5 / 15 | passed by every model or none — zero information |
| negative discrimination | 3 | better-overall models do worse — routed to adjudication, not deleted |
| **usable n** | **7** | items that are neither dead nor negatively discriminating |
| quotable? | **no** — below `usable_n ≥ 30` | whether a 95% interval on this axis should be published |

**This axis is not saturated.** A spread of 0.400 means it does separate models. But at
`usable_n = 7` it cannot yet resolve a difference worth reporting — a 95% Wilson interval
