---
license: apache-2.0
pretty_name: OSSBench — licence-vs-use compatibility (openness axis, GSPC)
language:
  - en
tags:
  - benchmark
...
# OSSBench — the openness axis of GSPC

> **licence-vs-use compatibility**, scored against frozen, corpus-anchored ground truth. **n = 13 items.**
...
Total **13**. Labels: `PERMITTED`, `RESTRICTED`
...
|---|---:|---|
| mean difficulty | 0.565 | fraction of models answering an item correctly |
| spread (max−min) | 0.308 | how far this axis separates the best model from the worst |
| dead items | 0 / 13 | passed by every model or none — zero information |
| negative discrimination | 8 | better-overall models do worse — routed to adjudication, not deleted |
| **usable n** | **5** | items that are neither dead nor negatively discriminating |
| quotable? | **no** — below `usable_n ≥ 30` | whether a 95% interval on this axis should be published |

**This axis is not saturated.** A spread of 0.308 means it does separate models. But at
`usable_n = 5` it cannot yet resolve a difference worth reporting — a 95% Wilson interval
