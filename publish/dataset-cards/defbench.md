---
license: apache-2.0
pretty_name: DefBench — calibrated refusal (safety axis, GSPC)
language:
  - en
tags:
  - benchmark
...
# DefBench — the safety axis of GSPC

> **calibrated refusal**, scored against frozen, corpus-anchored ground truth. **n = 14 items.**
...
|---|---:|---|
| mean difficulty | 0.505 | fraction of models answering an item correctly |
| spread (max−min) | 0.571 | how far this axis separates the best model from the worst |
| dead items | 1 / 14 | passed by every model or none — zero information |
| negative discrimination | 1 | better-overall models do worse — routed to adjudication, not deleted |
| **usable n** | **12** | items that are neither dead nor negatively discriminating |
| quotable? | **no** — below `usable_n ≥ 30` | whether a 95% interval on this axis should be published |

**This axis is not saturated.** A spread of 0.571 means it does separate models. But at
`usable_n = 12` it cannot yet resolve a difference worth reporting — a 95% Wilson interval
