---
license: apache-2.0
pretty_name: GovBench — EU AI Act risk tier (governance axis, GSPC)
language:
  - en
tags:
  - benchmark
...
# GovBench — the governance axis of GSPC

> **EU AI Act risk tier**, scored against frozen, corpus-anchored ground truth. **n = 24 items.**
...
Total **24**. Labels: `HIGH_RISK`, `LIMITED_RISK`, `MINIMAL_RISK`, `PROHIBITED`
...
|---|---:|---|
| mean difficulty | 0.227 | fraction of models answering an item correctly |
| spread (max−min) | 0.440 | how far this axis separates the best model from the worst |
| dead items | 1 / 24 | passed by every model or none — zero information |
| negative discrimination | 0 | better-overall models do worse — routed to adjudication, not deleted |
| **usable n** | **23** | items that are neither dead nor negatively discriminating |
| quotable? | **no** — below `usable_n ≥ 30` | whether a 95% interval on this axis should be published |

**This axis is not saturated.** A spread of 0.440 means it does separate models. But at
`usable_n = 23` it cannot yet resolve a difference worth reporting — a 95% Wilson interval
